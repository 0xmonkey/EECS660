...
def findCycle(G):  # {{{
    ...
    for x in range(0, len(G.adj)):
        ...
        if len(G[x]) == 0:
            ...
            prev = 0
            for y in G[x]:
                if x in predecessors(x_bfs[0], y):
                    u_path = common_ancestor_paths(
                        x_bfs[0], y, prev)[0][-1]
                    v_path = x == common_ancestor_paths(
                        x_bfs[0], y, prev)[0][-1]
                    if x == u_path and x == v_path:
                        return u_path
                    prev = y
