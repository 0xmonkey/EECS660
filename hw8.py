...
class priority_dict(dict):  # {{{1
    ...
    def peek(self):  # {{{
        ...
        while value not in self or self[value] != key:
            # only throws away outdated (key, value) pairs
            heappop(self._heap)
            key, value = self._heap[0]
        return value
    # --------------------------------------------------------------------------}}}
...
def Dijkstra(G, w, s):  # {{{
    # Dijkstra's shortest path algorithm. You might want to look at the MST_Prim
    # implementation above.
    path_weight = [-1 for x in G.nodes]
    path_weight[s] = 0
    T = AdjList(len(G))
    S = [s]
    while len(S) < len(G.nodes):
        # find the first crossing (u,v) in G s.t v not in s. Never going through entire list
        # unless there are no crossings left.
        for x in w:
            u = x[0]
            v = x[1]
            if v not in S and u in S:
                pwv = path_weight[u] + w[(u, v)]
                T[u].append(v)
                S.append(v)
                path_weight[v] = pwv
                break
...
Graph, w = rand_weight_graph(10)

res, p = Dijkstra(Graph, w, 3)
print(res)
